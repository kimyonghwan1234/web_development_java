...
public class Samsung {
    ...
    // 2. 기본 생성자 정의 ->접근 지정자 private도 주목
    private Samsung(){
        company = getClass().getSimpleName();    //클래스명을 가지고 오는 것
        serialNumber=20240000;
    }
    //3. static 메서드를 정의 ->대부분의 경우 getInstance()로 정의함    //public도 주목
    public static Samsung getInstance(){
        if(instance==null){ //현재 인스턴스가 없다면 이하의 코드가 실행됨.
            instance=new Samsung(); //삼성 클래스의 객체를 생성해
                                    //static 변수인 instance에 대입
        }
        //만약에 이상의 조건문이 실행되지 않는다면 이미 instance가 있다는 의미
        //즉, getInstance()메서드는 실행되기만 하면 어쨋든 Samsung클래스의 객체가
        //return 된다는 점을 확인할 수 있다.
        return instance;
    }
