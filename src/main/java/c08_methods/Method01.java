...
/*
    ...
    함수와 메서드가 동일한 개념이라면 왜 용어를 분리하는가?
        -> 클래스 내부에 함수를 정의하게 되면 이름 메서드라고 칭함.
    ...
    메서드 호출(call): main method가 있는 곳에서 클래스명.메서드명()혹은.
        ...
        컴퓨터는 시키는 대로만 하기때문에 1-5)를 입력해야함. 하지만, '사진을 찍는다'라는 메서드 내에 1-5)명령어들을
        미리입력해놀고나서, '사진을 찍는다'라는 명령어를 실행시키면 자동으로1-5)까지의 명령을 순서대로 수행함
        ->즉 메서드란 순차적으로 이루어지는 명령어들을 모아둔 것

        2.메서드 관련 용어 정리
            1)메서드 정의(define): 사용자가 메서드를 새로 만드는 것을 의미
            2)인수(argument): 사용자가 메서드에 전달할 입력을 의미
            3)매개변수(parameter): 인수를 받아서 저장하는 변수를 의미
            4) 반환값/출력값/리턴값(return): 사용자가 메서드의 출력을 의미
...
//        call2(routine);
        //이하의 방법으로는 return result를 콘솔에서 확인할 수 업승ㅁ
        //call3();
        //별찍기, 즉 return값을 출력하기 위해 출력하라는 명령어를 집어 넣어야만 함.
//        System.out.println(call3());

//        System.out.println(call4(2024, 11, 29));  // 실행 방법 1: 출력문 ㄴ내에 바로 함수 호출
        //변수 선언 및 초기화 -> 메서드의 return값을 todayDate에 대입
//    String todayDate= call4(2024, 11, 29);
//        System.out.println(todayDate); //실행방법2 : 별도의 변수를 선언하여 메서드의 return값을 저장 후, 변수를 통해 sout 실행
