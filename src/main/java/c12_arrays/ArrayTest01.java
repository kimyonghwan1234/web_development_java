...
/*
    배열의 필요성 :
    여태까지의 수업을 기준으로 하면 10개의 정수형 데이터를 저장하려면 int 변수를 10개 선언해서 저장해야했음
    배열은 '같은' 자료형의 데이터를 하나의 변수에 저장해서 처리하는 방식

    선언방식:
    자료형[] 변수명=
    ex)
    int[] arr = new int[5];         //배열 선언 방식1 -> 배열을 선언만 하고 후에 값을 대입

    int[] arr2 = {1,2,3,4,5};        //배열 선언 방식2 -> 배열 선언 및 초기화
    System.out.println(arr);
        System.out.println(arr2);
        을 출력해보면 [I@36baf30c, [I@7a81197d와 같은 값이 출력되고 [1,2,3,4,5]와 같은 방식으로 출력x

        여기서 일반 변수와 참조 변수 개념에 대해 짚고 넘어갈 필요가 있다.

        일반 변수: 데이터를 직접 가지는 변수
            byte, short, int, double등 기본 데이터 타입(자료형)을 가지는 경우
...
//        }
        ...
        // arr 객체의 필드인 length를 활용하여 컴퓨터가 알아서 배열의 크기를 찾아서 반복문을 통해 출력하도록 함
