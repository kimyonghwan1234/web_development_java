...
public class CentralControl {
    ...
    public void powerOff2() {
        ...
        for (Power device : deviceArray) {      //deviceArray라는 배열의 각 요소에 device라는 객체명을 임시로 붙임
            if (device == null) {          //null이면 continue
                continue;
            }
            device.off();
        }
    ...
    //displayInfo()에 해당하는 메서드를 작성해야 함 ->배열의 요소를 하나씩 뽑아내는 것도 가능하고, 배열 전체를 출력하여도 됨.

    //코드 내부에 sout을 구현하겠다는 의미 -> void보면
    public void displayInfo() {
        //내부에 sout구현
        //배열이 가득차지 않았을 경우를 대비해야함
        //각 요소들을 하나씩 뽑게 될 예정인데 그 경우 null 값을 만나게 되면 컴파일 오류가 생기기 때문
        //배열 내부 요소를 하나씩 뽑게 되는 방식이 뭐였는지 떠올려야 함
    ...
    // 배열 내부를 돌면서 각 요소의 고유 메서드들을 호출하는 예제 메서드
    public void performSpecificMethod() {
        for (Power device : deviceArray) {
            if (device instanceof Computer) {
                //deviceArray 내부에 있는 요소라면 기본적으로 Power 인터페이스의 객체이거나 혹은객체
                ...
                mouse.leftClick();  //LED클래스에 changeColor()메서드를 정의하고,
                //sout("색쌀을 바꿉니다")라고 한 후에 다운캐스팅을 하세요
